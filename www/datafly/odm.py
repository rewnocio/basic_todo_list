...
class Document(dict):
    ...
    def reload(self):
        doc = self.coll.find_one({'_id': self['_id']})
        self.update(doc)

    def save(self, hooks=True, *args, **kwargs):        
        if hooks:
            new = '_id' not in self
            self.pre_insert() if new else self.pre_update()                
            self.pre_save()
        self.coll.save(self, *args, **kwargs)
        if hooks:
            self.post_insert() if new else self.post_update()                
            self.post_save()
